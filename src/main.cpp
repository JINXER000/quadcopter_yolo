...
int main()
{
    ...
    Detector detector("cfg/yolov3-tiny.cfg", "cfg/yolov3-tiny.weights");
    Template tracker;
    ...
    while( capture.read(frame) )
    {
        ...
        if(status == 0)
        {
            vector<bbox_t> results = detector.detect(frame, 0.25);          
            ...
            if(persons.size() > 0)
            {
                ...
                rectangle(frameShow, location, Scalar(0, 0, 255), 5);
